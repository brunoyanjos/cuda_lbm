...
for sim_id, line, color_x, label in zip(sim_ids, linestyles,colors_x,labels):
    ...
    dummy_handle, = plt.plot([], [], linestyle=line, color='black', label=f"BC type: {label}")
...
plt.tight_layout()
plt.savefig('comparison_velocity_profiles_RBC.pdf', dpi=300, format='pdf', bbox_inches='tight')
plt.show()