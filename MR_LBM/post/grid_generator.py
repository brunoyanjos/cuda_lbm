    ...
    for j in range(Ny_fine_grid):
        ...
        if x % 1 == 0 and y % 1 == 0:
            if (
                ...
                x >= start_of_coarse - Ni
                and x <= stop_coarse + Ni
                and y >= start_of_coarse - Ni
                and y <= stop_coarse + Ni
            ):
...
print(
    f"total_fine_points: {2 * (Ny_fine_width * Nx_fine_grid + Nx_fine_width * (Ny_fine_grid - 2 * Ny_fine_width))}"
)
