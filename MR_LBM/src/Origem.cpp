...
//#endif
//
//__host__ void saveFluct(dfloat* ux, dfloat* uy, dfloat* uz, unsigned int nSteps
//
//)
//{
//    // Names of files
...
//
//    strFileUx = getVarFilename("uxFluct", nSteps, ".bin");
//    strFileUy = getVarFilename("uyFluct", nSteps, ".bin");
//    strFileUz = getVarFilename("uzFluct", nSteps, ".bin");
//    // saving files
//    int step = 0;
//    step = nSteps;
//    // if (step == N_STEPS){
...
//    dfloat* Sxx, dfloat* Sxy, dfloat* Sxz, dfloat* Syy, dfloat* Syz, dfloat* Szz,
...
//    BC_FORCES_PARAMS_DECLARATION(h_) unsigned int nSteps)
//{
//
//    // linearize
//    size_t indexMacr;
...
//
//#if STRAIN_RATE_TENSOR
//                mxx[indexMacr] = h_fMom[idxMom(x % BLOCK_NX, y % BLOCK_NY, z % BLOCK_NZ, M_MXX_INDEX, x / BLOCK_NX, y / BLOCK_NY, z / BLOCK_NZ)];
//                mxy[indexMacr] = h_fMom[idxMom(x % BLOCK_NX, y % BLOCK_NY, z % BLOCK_NZ, M_MXY_INDEX, x / BLOCK_NX, y / BLOCK_NY, z / BLOCK_NZ)];
//                mxz[indexMacr] = h_fMom[idxMom(x % BLOCK_NX, y % BLOCK_NY, z % BLOCK_NZ, M_MXZ_INDEX, x / BLOCK_NX, y / BLOCK_NY, z / BLOCK_NZ)];
//                myy[indexMacr] = h_fMom[idxMom(x % BLOCK_NX, y % BLOCK_NY, z % BLOCK_NZ, M_MYY_INDEX, x / BLOCK_NX, y / BLOCK_NY, z / BLOCK_NZ)];
//                myz[indexMacr] = h_fMom[idxMom(x % BLOCK_NX, y % BLOCK_NY, z % BLOCK_NZ, M_MYZ_INDEX, x / BLOCK_NX, y / BLOCK_NY, z / BLOCK_NZ)];
//                mzz[indexMacr] = h_fMom[idxMom(x % BLOCK_NX, y % BLOCK_NY, z % BLOCK_NZ, M_MZZ_INDEX, x / BLOCK_NX, y / BLOCK_NY, z / BLOCK_NZ)];
//
//                Sxx[indexMacr] = (as2 / (2 * TAU)) * (ux[indexMacr] * ux[indexMacr] - mxx[indexMacr]);
//                Sxy[indexMacr] = (as2 / (2 * TAU)) * (ux[indexMacr] * uy[indexMacr] - mxy[indexMacr]);
//                Sxz[indexMacr] = (as2 / (2 * TAU)) * (ux[indexMacr] * uz[indexMacr] - mxz[indexMacr]);
//                Syy[indexMacr] = (as2 / (2 * TAU)) * (uy[indexMacr] * uy[indexMacr] - myy[indexMacr]);
//                Syz[indexMacr] = (as2 / (2 * TAU)) * (uy[indexMacr] * uz[indexMacr] - myz[indexMacr]);
//                Szz[indexMacr] = (as2 / (2 * TAU)) * (uz[indexMacr] * uz[indexMacr] - mzz[indexMacr]);
//
//#endif
//#if TOTAL_KINETIC_ENERGY
//                uSum = uSum + (ux[indexMacr] * ux[indexMacr] + uy[indexMacr] * uy[indexMacr] + uz[indexMacr] * uz[indexMacr]) * (0.5);
//#endif
//
//#ifdef NON_NEWTONIAN_FLUID
...
//    filename_stream << "tKE_" << ID_SIM << ".dat";
//    std::string filename = filename_stream.str();
//
//    uSum = uSum / (U_MAX * U_MAX * NX * NY * NZ);
//    uSum = uSum / 9;
//    t_star = (nSteps * U_MAX) / NX;
//
//    std::ofstream data_file(filename.c_str(), std::ios::app);
//
//    data_file << t_star << " " << uSum << std::endl;
//    data_file.close();
//#endif
//
...
//    // Names of files
//    std::string strFileRho, strFileUx, strFileUy, strFileUz;
//    std::string strFileSxx, strFileSxy, strFileSxz, strFileSyy, strFileSyz, strFileSzz;
...
//
//#if STRAIN_RATE_TENSOR
//    strFileSxx = getVarFilename("Sxx", nSteps, ".bin");
//    strFileSxy = getVarFilename("Sxy", nSteps, ".bin");
//    strFileSxz = getVarFilename("Sxz", nSteps, ".bin");
//    strFileSyy = getVarFilename("Syy", nSteps, ".bin");
//    strFileSyz = getVarFilename("Syz", nSteps, ".bin");
//    strFileSzz = getVarFilename("Szz", nSteps, ".bin");
//#endif
//
//#ifdef NON_NEWTONIAN_FLUID
...
//        saveVarBin(strFileUz, uz, MEM_SIZE_SCALAR, false);
//#if STRAIN_RATE_TENSOR
//        saveVarBin(strFileSxx, Sxx, MEM_SIZE_SCALAR, false);
//        saveVarBin(strFileSxy, Sxy, MEM_SIZE_SCALAR, false);
//        saveVarBin(strFileSxz, Sxz, MEM_SIZE_SCALAR, false);
//        saveVarBin(strFileSyy, Syy, MEM_SIZE_SCALAR, false);
//        saveVarBin(strFileSyz, Syz, MEM_SIZE_SCALAR, false);
//        saveVarBin(strFileSzz, Szz, MEM_SIZE_SCALAR, false);
//#endif
//
//#ifdef NON_NEWTONIAN_FLUID
...
//#endif
//    strSimInfo << "                 Complete North Boundaries: " << BOOL_TO_STRING(COMPLETE_NORTH_BOUNDARIES) << "\n";
//    strSimInfo << "                 Re: " << RE << "\n";
//    strSimInfo << "                 NX: " << NX << "\n";
//    strSimInfo << "                 NY: " << NY << "\n";
//    strSimInfo << "                 NZ: " << NZ << "\n";
...
//
//    if (step == REPORT_SAVE)
//    { // check if first time step to save data
